...
def init():
    ...
    return cr, cm, X_train_undersampled, y_train_undersampled, X_test, y_test
c_report, c_matrix, X_train_undersampled, y_train_undersampled, X_test, y_test = init()
        ...
        <div class="flow-wrap">
            <div class="flow-col">
                ...
                <div class="flow-row">
                    ...
                    <div class="flow-box flow-right">Target Variable<br>FrafFound_P</div>
                </div>
            </div>
        </div>
        """,
    ...
    st.markdown(
        ...
        <div class="flow-wrap">
            <div class="flow-col">
                ...
                <div class="flow-arrow">↓</div>
                <div class="flow-box flow-step">2. Preprocessing & Encoding</div>
                <div class="flow-arrow">↓</div>
                <div class="flow-box flow-step">3. Model Training & Tuning</div>
                <div class="flow-arrow">↓</div>
                <div class="flow-box flow-step">4. Interpretation & Risk Factors</div>
                <div class="flow-arrow">↓</div>
    ...
    for ele in ['FraudFound_P', 'Age', 'VehicleCategory', 'AccidentArea', 'VehiclePrice', 'AgeOfVehicle', 'BasePolicy', 'NumberOfSuppliments', "Fault"]:#, 'PastNumberOfClaims', 'AddressChange_Claim', 'Fault', 'Make', 'VehicleCategory', 'VehiclePrice', 'AgeOfVehicle', 'AccidentArea', 'BasePolicy', 'PolicyType', 'Deductible', 'NumberOfSuppliments', 'AgentType', 'PoliceReportFiled', 'WitnessPresent', 'Days_Policy_Claim', 'Days_Policy_Accident', 'WeekOfMonthClaimed', 'MonthClaimed', 'Month', 'DayOfWeekClaimed', 'DayOfWeek', 'Year']:
      ...
      if ele == 'Age':
        ...
        pass
          ...
          if st.checkbox("Show Chi2 test and cramerV value", key='chi2_' + ele):
...
if page == pages[4] : 
  ...
  with st.spinner("Loading data... please wait"):
    ...
    # heatmap of the confusion matrix with seaborn
    ...
    #
    #fig = px.line(x=[1,2,3], y=[10,20,30], title="Centered Plot")
...
if page == pages[5]:
    ...
    with col1:
        fig_fp = px.box(FP, x=1, title='False Positives', color_discrete_sequence=["#1f77b4"])
    ...
    with col3:
        fig_fn = px.box(FN, x=1, title='False Negatives', color_discrete_sequence=["#d62728"])
        fig_fn.update_layout(height=400)
        st.plotly_chart(fig_fn, use_container_width=True)
    
    st.markdown("""
                <div style='padding: 12px; border-radius: 8px; background-color: #f8f9fa; border: 1px solid #ddd; margin-bottom: 20px;'>
                    ...
                    <p style='font-size: 15px; line-height: 1.5;'>
                        ...
                        <strong style='color:#1f77b4;'>False Positives</strong> represent legitimate claims that received high predicted fraud scores, indicating overestimation of fraud risk.  
                        ...
                        <strong style='color:#d62728;'>False Negatives</strong> correspond to fraudulent claims that were assigned low fraud probabilities, revealing systematic underestimation by the model.  
                        Together, these distributions illustrate how the decision threshold shapes model sensitivity and highlight regions where misclassification is most likely to occur.
                    </p>
                </div>
                """, unsafe_allow_html=True)
    ...
    if st.button("Show SHAP values", type='primary'): 
      ...
      st.markdown("""
                  <div style='padding: 15px; border-radius: 10px; background-color: #f7f7f7; border: 1px solid #ddd;'>

                  <h3 style='color:#333;'>🔍 Key Insights from Feature Influence</h3>
...
.css-1d391kg input, .css-1d391kg textarea {
    background-color: #1B263B;
    color: #FFFFFF;
}
</style>
